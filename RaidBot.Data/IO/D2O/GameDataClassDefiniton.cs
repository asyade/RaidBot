...
namespace RaidBot.Data.IO.D2O
{
    public class GameDataClassDefiniton
    {
        ...
        public void SetField(Dictionary<int, GameDataClassDefiniton> classes, string I18nAccesorPath)
        {
            foreach (GameDataField field in Fields)
            {
                field.SetClasses(classes, I18nAccesorPath);
            }
        }

        public IData Read(BigEndianReader reader, string I18nAccesor)
        {
			if (classe == null)
		     return (IData)null; 
        ...
        private object GetValue(object value,Type t)
        {
            ...
            if (t.GetInterface("IList") != null)
            {
                ...
                foreach (var obj in objectList)
                {
                    if (obj != null)
                    {
                        if (genericType.GetInterface("IList") != null)
                        {
                            a_Context.Add(GetValue(obj, genericType));
                        }
                        else
                        {
                            if (obj.GetType().GetInterface("IConvertible") != null)
