    ...
    public class Server
    {
        ...
        public void Start(short listenPort)
        {
            if (!Runing)
            {
                ...
                RaidBot.Common.Default.Loging.Logger.Default.Log("Serveur en écoute .");
            }
            else
                RaidBot.Common.Default.Loging.Logger.Default.Log("Serveur déjà en écoute..." , RaidBot.Common.Default.Loging.LogLevelEnum.Error);
        }
